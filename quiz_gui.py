...
from quiz_logic import (  # Import from our logic module
    load_questions,
...
Add instruction helper in corner (just a button with a textbox)
"""
...
class PasteWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        submit_btn = ctk.CTkButton(self, text="Submit", command=self.submit_text)
...
class QuizApp(ctk.CTk):
    ...
    def show_start_screen(self):
        ...
        help_btn = ctk.CTkButton(
            self.main_frame, text="Help", command=self.helpUser, width=200, height=40
        )
        help_btn.pack(side="bottom", anchor="se", pady=10, padx=10)

    def helpUser(self):
        helperText = (
            "Before generating quiz questions with a chatbot, paste the following as a prompt to the AI\n\n\n"
            "From this moment on you are a quiz generator that outputs only raw ASCII "
            "blocks matching exactly the following format:\n\n"
            '"""QUESTION"""\n'
            ...
            "Repeat these blocks back-to-back with no extra whitespace, bullets, "
            "or added formatting."
        )
    ...
    def check_answer(self):
        ...
        if not self.selected_answer.get():
            ...
            self.feedback_label.configure(
                text="Please select an answer", text_color="yellow"
            )
            return
        ...
        if user_answer == correct:
            ...
            # FIXME change this to a message inside the application with a popup
            self.feedback_label.configure(
                text=(f"Incorrect. The correct answer is {correct}"), text_color="red"
            )
    ...
    def show_results(self):
        ...
        message_label = ctk.CTkLabel(
            self.main_frame, text=message, font=ctk.CTkFont(size=18)
        )
